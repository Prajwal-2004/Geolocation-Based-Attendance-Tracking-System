...
import { Menu, X, MapPin, Shield, Users, Zap } from 'lucide-react';

const GeoAttendLogo = ({ className = "h-10 w-10" }: { className?: string }) => (
  <svg viewBox="0 0 64 64" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
    ...
    {/* Location pin shape encompassing the globe */}
    <path
    ...
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      {menuOpen && (
        <div className="absolute top-16 right-6 z-30 w-56 rounded-2xl border border-border bg-card shadow-2xl p-2 animate-slide-up">
          <button
            ...
            className="w-full text-left px-4 py-3 rounded-xl text-sm font-medium hover:bg-secondary/60 transition-colors"
          >
            Sign In
          </button>
          <button
      ...
      <main className="relative z-10 flex flex-col items-center justify-center text-center px-6 pt-16 pb-24 max-w-3xl mx-auto">
        ...
        <p className="text-lg text-muted-foreground max-w-md leading-relaxed mb-10">
          Location-verified attendance tracking powered by polygon geofencing and anti-spoofing algorithms.
        </p>
        ...
        <div className="flex gap-3">
          <Button
            ...
            size="lg"
            className="h-12 px-8 rounded-xl font-semibold shadow-lg shadow-primary/20 text-sm"
          >
            Get Started
          </Button>
          <Button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-20 w-full">
          {[
            ...
            },
            {
              icon: Shield,
              title: 'Anti-Spoofing',
              desc: 'GPS accuracy checks and timestamp drift detection',
            },
            {
              icon: Users,
              title: 'Role-Based Access',
              desc: 'Separate flows for students, faculty, and administrators',
            },
          ].map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="rounded-2xl border border-border/40 bg-card/50 p-6 text-left hover:border-primary/30 transition-colors"
            >
